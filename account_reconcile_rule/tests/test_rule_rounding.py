...
class TestRuleRounding(AccountReconciliationModelTestCase):
        ...
        cls.rule_round_1 = cls.rule_obj.create(
            {
                "name": "Rounding -1.0 to 0.0",
                "rule_type": "rounding",
                "reconcile_model_ids": [(6, 0, (cls.reconcile_model_round_1.id,))],
                "amount_min": -1.0,
                "amount_max": 0,
                "sequence": 1,
            }
        ...
        cls.rule_round_2 = cls.rule_obj.create(
            {
                "name": "Rounding -1.0 to 0.0",
                "rule_type": "rounding",
                "reconcile_model_ids": [(6, 0, (cls.reconcile_model_round_2.id,))],
                "amount_min": -2.0,
                "amount_max": -1.0,
                "sequence": 2,
            }
        ...
        cls.rule_round_3 = cls.rule_obj.create(
            {
                "name": "Rounding 0.0 to 2.0",
                "rule_type": "rounding",
                "reconcile_model_ids": [(6, 0, (cls.reconcile_model_round_3.id,))],
                "amount_min": 0,
                "amount_max": 2,
                "sequence": 2,
            }
    ...
    def test_operations(self):
        ...
        statement_line, move_line = self.prepare_statement(-0.5)
        ops = self.rule_obj.models_for_reconciliation(statement_line.id, move_line.ids)
    ...
    def test_multicurrency_lines(self):
        ...
        statement_line, move_line = self.prepare_statement(
            -0.5, statement_line_currency=currency, move_line_currency=currency
        )
        ops = self.rule_obj.models_for_reconciliation(statement_line.id, move_line.ids)
