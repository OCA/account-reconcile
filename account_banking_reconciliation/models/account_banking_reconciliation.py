        ...
        for statement in self:
            ...
            if statement.cleared_balance_cur:
                if statement.difference_cur != 0.0:
                    raise UserError(
                        _(
                            "Prior to reconciling a statement, "
    ...
    def action_process(self):
        ...
        for statement in self:
            ...
            for statement_line in statement_lines:
                ...
                statement_id = (
                    statement_line.cleared_bank_account and statement.id or False
                )
        ...
        for statement in self:
            for line in statement.credit_move_line_ids:
                statement.sum_of_credits += (
                    line.cleared_bank_account
                    and float_round(line.amount, account_precision)
                    or 0.0
                )
                statement.sum_of_credits_cur += (
                    line.cleared_bank_account
                    and float_round(line.amountcur, account_precision)
                    or 0.0
                )
                statement.sum_of_credits_lines += line.cleared_bank_account and 1 or 0
                statement.sum_of_ucredits += (
                    (not line.cleared_bank_account)
                    and float_round(line.amount, account_precision)
                    or 0.0
                )
