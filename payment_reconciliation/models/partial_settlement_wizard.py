...
class PartialSettlementWizard(models.TransientModel):
    ...
    payment_id = fields.Many2one('account.payment', string="Payment", domain="[('partner_id', '=', partner_id), ('state', '=', 'posted'), ('is_reconciled', '=', False)]")

    total_invoice_due = fields.Monetary(string="Total Invoice Due", compute='_compute_invoice_due')
    currency_id = fields.Many2one('res.currency', related='partner_id.currency_id', readonly=True)

    total_to_reconcile = fields.Monetary(string="Total to Reconcile", compute='_compute_total_to_reconcile', store=True)
    payment_residual = fields.Monetary(string="Payment Available", compute='_compute_payment_residual', store=True)

    line_ids = fields.One2many('partial.settlement.line', 'wizard_id', string="Invoices")

    payment_amount = fields.Monetary(string="Payment Amount", compute='_compute_payment_amount')
    has_payments = fields.Boolean(string="Has Payments", compute="_compute_has_payments", store=False)

    @api.depends('partner_id')
    def _compute_has_payments(self):
        for rec in self:
            rec.has_payments = bool(self.env['account.payment'].search([
    ...
    def _onchange_line_ids_payment_filter(self):
        ...
        # Log or debug
        print(f"🔍 Detected {'Vendor' if is_vendor_context else 'Customer'} context based on line_ids")
        ...
        for rec in self:
            if rec.payment_id:
                ...
                for line in rec.payment_id.move_id.line_ids:
                    if line.account_id.account_type in ('asset_receivable',
    ...
    def action_reconcile(self):
        ...
        remaining_payment_amount = abs(sum(
            payment_lines.mapped(lambda x: x.amount_residual_currency or x.amount_residual)
        ))
        ...
        for line in self.line_ids.filtered(lambda l: l.partial_amount > 0):
            ...
            if not invoice_lines:
                print(f"⚠️ No residual invoice lines to reconcile for invoice {invoice.name}")
                continue
            ...
            debit_residual = debit_line.amount_residual_currency or debit_line.amount_residual
            credit_residual = credit_line.amount_residual_currency or credit_line.amount_residual
            ...
            try:
                ...
                if debit_line.amount_currency and credit_line.amount_currency:
                    reconcile_vals.update({
                        'debit_amount_currency': min(abs(amount), abs(debit_line.amount_currency)),
                        'credit_amount_currency': min(abs(amount), abs(credit_line.amount_currency)),
                ...
                print(f"✅ Successfully reconciled {amount} out of {invoice_residual} for invoice {invoice.name}")
...
class PartialSettlementLine(models.TransientModel):
    ...
    ], compute="_compute_document_type", store=True)

    document_type_label = fields.Html(string="Type", compute="_compute_document_type_label")
    ...
    def _compute_document_type_label(self):
        for line in self:
            if line.document_type == 'invoice':
                line.document_type_label = '<span style="color:green; font-weight:bold;">Invoice / Bill</span>'
