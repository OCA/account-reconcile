...
odoo.define(
    "account_reconciliation_widget.reconciliation_tests.data",
    function (
        ...
        var db = {
            ...
            "account.tax": {
                ...
                json_friendly_compute_all: function (args) {
                    ...
                    for (let i = 0; i < tax.invoice_repartition_line_ids.length; i++) {
                        var rep_ln = _.find(
                            db["account.tax.repartition.line"].records,
                            {
            ...
            "account.bank.statement.line": {
                ...
                records: [
                    {
                        id: 5,
                        display_name: "SAJ/2014/002 and SAJ/2014/003",
                        company_id: 1,
                    },
            ...
            "account.reconciliation.widget": {
                ...
                process_move_lines: function (args) {
                    ...
                    for (var i in datas) {
                        var data = datas[i];
                        for (var key in Datas.used
                            .move_lines_for_manual_reconciliation) {
                            Datas.used.move_lines_for_manual_reconciliation[key] =
                                _.filter(
                                    Datas.used.move_lines_for_manual_reconciliation[
                                        key
                                    ],
                                    function (mv_line) {
                                        return (
                                            data.mv_line_ids.indexOf(mv_line.id) === -1
                                        );
                                    }
                                );
                        }
        ...
        Datas.params = {
            ...
            data_for_manual_reconciliation_widget:
                data_for_manual_reconciliation_widget,
...
odoo.define("account_reconciliation_widget.reconciliation_tests", function (require) {
    ...
    QUnit.module(
        ...
        function () {
                ...
                testUtils.mock.addMockEnvironment(clientAction, {
                    ...
                    archs: {
                        "account.bank.statement.line,false,search":
                            '<search string="Statement Line"><field name="display_name"/></search>',
                    },
                });
            ...
            QUnit.test(
                "Reconciliation validate without proposition",
                async function (assert) {
                    ...
                    await testUtils.dom.click(
                        widget.$("button.o_validate:not(:hidden)")
                    );
            ...
            QUnit.test(
                "Reconciliation validate with proposition",
                async function (assert) {
                    assert.expect(1);
                    ...
                    // open the first line
                    await testUtils.dom.click(
                        widget.$(".accounting_view thead td:first"),
                        {
                            allowInvisible: true,
                        }
                    );
            ...
            QUnit.test(
                "Reconciliation partial [REQUIRE FOCUS]",
                async function (assert) {
                    assert.expect(8);
                    ...
                    assert.equal(
                        widget.$(".accounting_view tbody .cell_left .edit_amount")
                            .length,
                        1,
                        "should display the edition pencil"
                    );
                    ...
                    assert.equal(
                        clientAction.widgets[1].$("caption button:disabled:visible")
                            .length,
                        1,
                        "button should be disabled"
                    );
                ...
                testUtils.mock.addMockEnvironment(clientAction, {
                    ...
                    archs: {
                        "res.partner,false,list":
                            '<tree string="Partners"><field name="display_name"/></tree>',
            ...
            QUnit.test(
                "Reconciliation create line (many2one test)",
                async function (assert) {
                    assert.expect(5);
                    ...
                    testUtils.mock.addMockEnvironment(clientAction, {
                        ...
                        archs: {
                            "account.account,false,list":
                                '<tree string="Account"><field name="code"/><field name="name"/></tree>',
                            "account.account,false,search":
                                '<search string="Account"><field name="code"/></search>',
                            "account.bank.statement.line,false,search":
                                '<search string="Statement Line"><field name="display_name"/></search>',
                        },
                        mockRPC: function (route, args) {
                    ...
                    await testUtils.dom.click(
                        widget.$(".create .create_account_id input")
                    );
            ...
            QUnit.test(
                "Reconciliation create line with taxes",
                async function (assert) {
                    assert.expect(13);
            ...
            QUnit.test(
                "Reconciliation: payment and 2 partials",
                async function (assert) {
                    assert.expect(6);
            ...
            QUnit.test(
                "Manual Reconciliation: No lines for account",
                async function (assert) {
                    assert.expect(2);
            ...
            QUnit.test(
                "Reconcile temporarily and ask to check",
                async function (assert) {
                    assert.expect(4);
                    ...
                    assert.equal(
                        $("button.o_validate.btn.btn-secondary.text-warning:first")
                            .length,
                        0,
            ...
            QUnit.test(
                "Reconciliation Models handle analytic tags",
                async function (assert) {
                    assert.expect(6);
