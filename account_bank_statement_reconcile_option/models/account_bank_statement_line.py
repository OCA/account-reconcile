...
class AccountReconciliation(models.AbstractModel):
    ...
    def _get_move_line_reconciliation_proposition(self, account_id, partner_id=None):
        ...
        query = f"""
            ...
            AND a.id IN (SELECT id FROM {from_clause + where_str})
            AND b.id IN (SELECT id FROM {from_clause + where_str})
        ...
        if st_line.journal_id.reconcile_mode == "journal_account":
            ...
            if bank_reconcile_account_allowed_ids:
                reconciliation_account_all = (
                    aml_accounts + bank_reconcile_account_allowed_ids
                )
                args += [
    ...
    def get_date_additional_domain(self, st_line):
        ...
        search_limit_days = (
            ...
            and st_line.statement_id.journal_id
            and st_line.statement_id.journal_id.search_limit_days
            or 0.0
        )
        if date_string and search_limit_days:
            limit_days_after = date_string + timedelta(days=search_limit_days)
            limit_days_before = date_string - timedelta(days=search_limit_days)
            domain = [("date", "<", limit_days_after), ("date", ">", limit_days_before)]
            return domain
        else:
            return []
        ...
        for line in bank_statement_lines:
            if matching_amls[line.id].get("status") == "reconciled":
                ...
                line_vals = {
                    ...
                    and self.with_context(journal_ids=journal_ids)._prepare_move_lines(
                ...
                if (
                    line.journal_id.set_default_reconcile_partner
                    and not line.partner_id
                    and partner_map.get(line.id)
                ):
    ...
    def _prepare_move_lines(
        ...
        for ret_line in ret:
            if ret_line["account_code"] in list_account_code:
                ret_line.update({"already_paid": True})
            else:
                ret_line.update({"already_paid": False})
        return ret
